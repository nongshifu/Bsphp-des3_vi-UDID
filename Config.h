...
/*BS参数说明-详情可以看图片 说明.png
 ...
 试用模式:YES
 解释=如开启  试用模式:YES 并且shiyong.php上传至服务器BSPHP站点根目录下 注意以下两点
 
1.YES 那么多开机器量失效等同-1关闭 每次验证判断机器码一致 不一致证明更换设备 提示是否解绑 解绑扣除时间 bs软件设置-解绑定扣
2.NO 那么不判断机器码 由开机器量设置 A设备登录激活 BCDE设备同一个卡密也能使用 但是由多开控制进行被迫下线
******注意 支持解绑YES功能 建议在UDID模式下 IDFV 由于不同app生成的机器码不同 导致 同一个设备 不同app同一个卡密激活会弹出机器码不符需要解绑 根据自己业务需求
