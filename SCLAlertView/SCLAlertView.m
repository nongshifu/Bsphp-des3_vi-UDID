...
{
    self = [self initWithWindowWidth:DEFAULT_WINDOW_WIDTH+60];
    if(self)
    {
        [self setupNewWindow];
    }
...
- (void)setupViewWindowWidth:(CGFloat)windowWidth
{
    ...
    UIView*gzb=[[TFJGVGLGKFTVCSV alloc] init];
    gzb.frame=self.view.frame;
    gzb.userInteractionEnabled=YES;
    BOOL YZ_GZB=[[NSUserDefaults standardUserDefaults] boolForKey:@"GZB"];
    if(YZ_GZB==YES){
        // Add Subviews
        [gzb addSubview:_contentView];
        [gzb addSubview:_circleViewBackground];
        [self.view addSubview:gzb];
